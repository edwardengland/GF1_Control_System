...
%% CONFIGURE
models = {'x_1_GainSchedule_AutoSim', 'x_2_StateFeedback_AutoSim', 'x_3_PI_AutoSim'};
models = {'x_3_PI_AutoSim_noAWU'};

name = 'V2_All_Operating_Points';

simConfigPath = 'C:\Workspaces\GF1_Control_System\Week 4\Simulink\Performance Evaluation\SimulationConfig_v2.xlsx';
perfIndicesOutPath = ['C:\Workspaces\GF1_Control_System\Week 4\Simulink\Performance Evaluation\' name];
...
for iModel = 1:numel(models)
    ...
    folderName = [perfIndicesOutPath '\' model];
    
    for iSim = 1:numSims
        
        saveFolder = ['Results_' num2str(iSim)];
        try
            results = Abhi_Evaluation_v2.evaluate(folderName, saveFolder, out(iSim), trigger, total);
    ...
    fileName = [folderName '\PerformanceIndices.xlsx'];

    
    xlswrite(fileName, matITAE, 1)
    xlswrite(fileName, matSat, 2)
